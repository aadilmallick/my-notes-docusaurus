...
You can also override or supplement variables locally with an `env.json` file:

```bash
sam local start-api --env-vars env.json
```

This lets you use local database URLs or API keys without modifying `template.yaml`.
#### Logs
...
1. Create a VSCode launch profile for SAM tasks
2. Run `sam local invoke` but attach the Node inspector to a specific port.

```bash
sam local invoke -d 9229
```

Then VS Code can attach directly to the Lambda process, allowing you to:

- Set breakpoints
- Inspect variables
- Step through your TypeScript (with source maps)
...
#### Watching deployments development

If you want local changes to your SAM tempalte or lambda source code to be immediately deployed, run this command:
...
This is the basic dockerfile for a containerized lambda.
...
1. `FROM public.ecr.aws/lambda/nodejs:24`: uses a dedicated lambda image
2. `COPY app.mjs package*.json ./`: copy all files
3. `RUN npm install`: install dependencies, but you should really run `npm ci` when deploying to production.
4. `CMD ["app.lambdaHandler"]`: says to run the specific handler destination for the lambda.

Then SAM just runs `docker build` when building the application.

#### Docker with TypeScript

To use docker with typescript, it involves just two simple steps:

1. Setup the node code to use typescript with a `tsconfig.json`, install typescript as a dev dependency, and include a build script to compile TS to JS.
2. In the docker file, run the build script to compile TS to JS.

Here is an appropriate dockerfile:

```dockerfile
FROM public.ecr.aws/lambda/nodejs:22

# 1. equivalent to cd /var/task
WORKDIR ${LAMBDA_TASK_ROOT}

# 2. copy over package.json and package-lock.json to take advantage of layers
COPY package*.json ./

# 3. npm ci for production builds
RUN npm ci

# 4. copy over rest of files
COPY . .

# 5. compile TS to JS
RUN npm run build

# 6. run outputted lambda
CMD ["dist/app.handler"]
```

But you could also use a multi-stage build so you can get rid of bloat like dev dependencies (TypeScript) and output just the barebones lambda distributable at the end.
...
- `packages/shared`: shared utilities (types, zod schemas, utils) that you use the `make` tool to copy into each docker build context for the lambda functions you're creating
- `functions`: group all lambda function folders into this directory

And then for an individual function, here is how you should define your code:

```bash
functions/
└── create-user/
    ├── src/
    │   ├── handler.ts # actual handler
    │   ├── service.ts # business logic
    │   ├── repository.ts # abstract and concrete DB following repository pattern
...
- **`handler.ts`** – Lambda entry point, parse request, return response.
- **`schema.ts`** – Zod validation.
- **`service.ts`** – Business logic.
- **`repository.ts`** – Database access.
- **`types.ts`** – Shared types for the function.
