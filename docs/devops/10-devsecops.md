...
2. The `before_script` directive runs before any job, so use it to view package version info and install packages
...
3. Create a list of stages in sequential order, such that jobs belonging to the same stage can execute in parallel.
...
4. In the build job, make the `node_modules` folder an artifact so later stages can access dependencies.
...
6. Run security tests, fail based on result of docker logs, which you can do via bash:
	- `exit 0`: success
	- `exit 1`: failure
