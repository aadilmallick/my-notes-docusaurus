...
The Chrome DevTools MCP lets AI agents like Claude interact directly with your running browser. It gives the AI "eyes" to take screenshots, read console messages, inspect network requests, and record performance traces.
...
Make sure to start a chrome instance with remote debugging available for this to work:
...
ALSO USE TAILWIND, change tailwind config and variables according to design principles in this skill, use it for everything, only escape to CSS for extremely custom things
