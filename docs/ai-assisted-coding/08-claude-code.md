...
Monolithic prompts are fragile. Here's an example:
...
A good `tasks-spec.md` shgould have the following:

1. List of tasks where each task has this info:
	- **dependency order**: how tasks depend on each other and the order they should be completed in.
	- **goal**: the goal of this task
	- **input**: the main data input that this feature will deal with
	- **output**: the main data structure this feature should output
	- **acceptance criteria**: the requirements that this feature should have
	- **test command**: the bash command to test this feature
	- **commit message**: the git bash command to commit this code with a specific message once done.

````md title="tasks-spec.md"
